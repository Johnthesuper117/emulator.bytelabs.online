...
// ===== START APPLICATION =====
// Initialize search UI on load. The legacy boot/menu is retained in this file
// but we won't run the boot sequence so the search UI (in index.html) remains.
...
async function initSearch() {
    try {
        ...
        gamesDatabase = Array.isArray(json) ? json : (json.games || []);
    } catch (err) {
        // Fallback: try reading embedded script tag if fetch fails
        try {
            const embedded = document.getElementById('games');
            if (embedded) gamesDatabase = JSON.parse(embedded.textContent || '[]');
        } catch (e) {
    ...
    // Initialize Fuse with title and tags (user requested)
    fuse = new Fuse(gamesDatabase, {
        keys: ['title', 'tags'],
        threshold: 0.35,
        includeScore: true
    });
...
function onSearchKeyDown(e) {
    ...
    if (e.key === 'ArrowDown') {
        ...
        if (selectedIndex >= 0 && currentResults[selectedIndex]) {
            launchGame(currentResults[selectedIndex].item);
        } else if (currentResults.length > 0) {
            // Launch first result if Enter pressed without explicit selection
            launchGame(currentResults[0].item);
        } else {
            // No results: go to emulator to use your own ROM
...
function displayResults(results) {
    ...
    if (!results || results.length === 0) {
        ...
        // Show option to use your own ROM
        const own = document.createElement('div');
        own.className = 'game-item';
        own.innerHTML = `<div class="game-title">Use your own ROM</div><div class="game-description">Open emulator with a ROM URL or upload a file</div>`;
        own.addEventListener('click', () => location.href = 'emulator.html');
        resultsContainer.appendChild(own);
        return;
    }

    // Header
    const header = document.createElement('div');
    header.className = 'terminal-line';
    header.textContent = `FOUND ${results.length} GAME(S):`;
    resultsContainer.appendChild(header);
    const sep = document.createElement('div');
    sep.className = 'terminal-line';
    sep.textContent = '═══════════════════════════════════════════════════';
    ...
    results.forEach((result, index) => {
        ...
        gameDiv.innerHTML = `
            ...
            <div class="game-meta">${game.platform || ''} ${game.year ? '| ' + game.year : ''} ${game.genre ? '| ' + game.genre : ''}</div>
    ...
    // Add final option: Use your own ROM
    const own = document.createElement('div');
    own.className = 'game-item';
    own.style.marginTop = '12px';
    own.innerHTML = `<div class="game-title">Use your own ROM</div><div class="game-description">Open emulator to paste a ROM URL or upload a file</div>`;
    own.addEventListener('click', () => location.href = 'emulator.html');
    resultsContainer.appendChild(own);
}
